...
CREATE TABLE DESCARGABLE (
    ID_CONTENIDO INT NOT NULL
    ,PRIMARY KEY (ID_CONTENIDO)
    ,constraint FK_DESCARGABLE FOREIGN KEY(ID_CONTENIDO) REFERENCES CONTENIDO(ID_CONTENIDO) ON DELETE CASCADE ON UPDATE CASCADE
);
...
CREATE TABLE MUSICA(
    ...
    ,CALIDAD     VARCHAR(50)
    ,PRIMARY KEY (ID_CONTENIDO)
    ,constraint FK_MUSICA FOREIGN KEY(ID_CONTENIDO) REFERENCES DESCARGABLE(ID_CONTENIDO) ON DELETE CASCADE ON UPDATE CASCADE
);
...
CREATE TABLE DOCUMENTO(
    ...
    ,PRIMARY KEY (ID_CONTENIDO)
    ,constraint FK_DOCUMENTO FOREIGN KEY(ID_CONTENIDO) REFERENCES DESCARGABLE(ID_CONTENIDO) ON DELETE CASCADE ON UPDATE CASCADE
);
...
CREATE TABLE MIRA(
    ...
    ,ID_CONTENIDO       INT  NOT NULL
    ,ID_USUARIO         INT NULL
    ,PRIMARY KEY(ID_VISUALIZACION,ID_CONTENIDO)
    ,FOREIGN KEY (ID_VISUALIZACION)
		REFERENCES VISUALIZACION_ONLINE(ID_VISUALIZACION)
        ON DELETE CASCADE
        ON UPDATE CASCADE
    ,FOREIGN KEY (ID_CONTENIDO)
        REFERENCES VIDEO(ID_CONTENIDO)
        on delete CASCADE
        ON UPDATE CASCADE
    ,FOREIGN KEY (ID_USUARIO)
        REFERENCES USUARIO(ID_USUARIO) 
...
CREATE TABLE RTA_ENCUESTA(
    ...
    ,FOREIGN KEY (ID_USUARIO)
        REFERENCES DESCARGA(ID_USUARIO)
